...
def plot_training_history(history):
    ...
    ax.semilogy(history['total'], 'k-', linewidth=2, label='Total Loss')
    ax.semilogy(history['pde'], 'b-', linewidth=1.5, label='PDE Residual Loss')
    ax.semilogy(history['ic'], 'r-', linewidth=1.5, label='IC Loss')
    ax.semilogy(history['bc0'], 'g-', linewidth=1.5, label='BC0 Loss')
...
def main(cfg: DictConfig):
    ...
    fig2,fig_err = plot_solution(model, theta_gt_test = 0.95238, theta0_0_test = 0.829, source_amplitude = cfg.source.source_amplitude, gt=gt_data)
