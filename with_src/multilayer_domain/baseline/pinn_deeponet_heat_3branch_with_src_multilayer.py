...
class PINNDeepONet(nn.Module):
    ...
    def compute_coefficients(self, x, L0 = 0.07, t_span = 1800.035, c = [2348,3421], ro = [911,1090], k = [0.21,0.49],
        ...
        # corrected coefficient per each x
    ...
    def train_pinn(self, n_epochs=5000, n_colloc=200, n_bc0 = 40, n_therm = 50,lr=1e-3,
                   source_type='gaussian', source_amplitude=0.0,Tx_range=(0.3,0.8),x_meas=0.5):
        """
...
# ------- Plotting ------- #
def plot_solution(model, Tx_test, x_meas_test, source_type='gaussian', source_amplitude=0.5, gt=None):
    """Visualize the trained solution"""
    
    # Generate test case
    ic_sensors = model.generate_ic()
    ...
    ax1.axvline(0.5, color='gray', linestyle='--', linewidth=2)  # Interface point
...
def main(cfg: DictConfig):
    ...
    history = model.train_pinn(
        ...
        Tx_range=cfg.training.Tx_range_multilayer,
    ...
    fig3,fig_err,fig_4 = plot_solution(model,Tx_test = cfg.test.Tx_test_multilayer, x_meas_test = cfg.test.x_meas_test,
                                       source_type=cfg.source.source_type, source_amplitude = cfg.source.source_amplitude_multilayer, gt=gt_data)
