...
class PINNDeepONet(nn.Module):
    ...
    def __init__(self, n_sensors_ic=20, n_sensors_src=20,n_sensors_thermal=20, 
        ...
        self.register_buffer('sensor_t_thermal_meas', 
                           torch.linspace(self.domain[0], self.domain[1], n_sensors_thermal))
    
    def forward(self, xt, src_sensors, thermal_sensors):
        """
        Args:
        ...
        single_sample = src_sensors.dim() == 1
        if single_sample:
        ...
        b_combined = b_src + b_meas              # (batch, p)
    ...
    def compute_pde_residual(self, xt, src_values, thermal_sensors, src_sensors, a1=18.992, a2=34185.667, wb=0.0005):
        """
        ...
        # Forward pass
        u = self.forward(xt_requires_grad, src_sensors, thermal_sensors)
    ...
    def compute_bc0_residual(self, src_sensors, thermal_sensors, xt, a5=175):
        """
    ...
    def compute_thermal_meas_residual(self,src_sensors, thermal_sensors,x_meas,xt,Tx):
        """
        ...
        u_meas = self.generate_internal_measurements(Tx, [0,1],t)
        
        # Forward pass
        u = self.forward(xt, src_sensors,thermal_sensors) 

        # Thermal measurement residual: u(x_meas) - analytical solution
        u_meas_pred = u[xt[:, 0] == x_meas]
        residual = u_meas - u_meas_pred
        return residual       
    
    def compute_interface_continuity_loss(self, amplitude, source_type ,a3,a4, Tx,xit):
        """
        Compute interface loss at material interfaces.
        
        Args:
            ic_sensors: (n_sensors_ic,) IC measurements
            src_sensors: (n_sensors_src,) source measurements
            xt_interface: (n_interface_points, 2) interface points [x, t]
            a1_left, a2_left: coefficients on the left side of the interface
            a1_right, a2_right: coefficients on the right side of the interface
        
        Returns:
            interface_continuity_loss: scalar tensor
        """

        # Enable gradients for xt_interface
        xt_requires_grad = xit.clone().requires_grad_(True)
        
        # Source sensors
        src_sensors_left = self.generate_source(source_type,amplitude, a3[0], a4[0], subdomain=(self.domain[0],self.domain[1]/self.n_layers))
        src_sensors_right = self.generate_source(source_type,amplitude, a3[1], a4[1], subdomain=(self.domain[1]/self.n_layers, self.domain[1]))
        
        # Thermal sensors
        thermal_sensors_left = self.generate_internal_measurements(Tx[0], subdomain=(self.domain[0],self.domain[1]/self.n_layers))
        thermal_sensors_right = self.generate_internal_measurements(Tx[1], subdomain=(self.domain[1]/self.n_layers, self.domain[1]))
        
        # Forward pass
        u_left = self.forward(xt_requires_grad, src_sensors_left, thermal_sensors_left)       
        u_right = self.forward(xt_requires_grad, src_sensors_right, thermal_sensors_right)
        residual = ((u_left - u_right))
        
        return residual
    
    def compute_flux_loss(self, amplitude, source_type, a3, a4, Tx, xit, k = [0.21,0.49]):
        """
        Compute flux continuity loss at material interfaces.
        ...
        Returns:
            flux_continuity_loss: scalar tensor
        """
        ...
        # Compute u_x at interface points
        u_x_left = torch.autograd.grad(u_left, xt_requires_grad,
                                  torch.ones_like(u_left),
                                  create_graph=True)[0][:, 0]  # ∂u/∂x
        u_x_right = torch.autograd.grad(u_right, xt_requires_grad,
                                  torch.ones_like(u_right),
                                  create_graph=True)[0][:, 0]  # ∂u/∂x

        # Flux continuity residual
        residual = ((k[0] * u_x_left - k[1] * u_x_right))
        
        return residual
    
    
    def generate_source(self,source_type='gaussian', amplitude=0.5, a3 = 0.7939, a4 = 3.570, subdomain=(0, 1), x = None):
        """
        ...
        """
        if x is None:
            x = torch.linspace(subdomain[0], subdomain[1], self.n_sensors_src)

        # Evaluate source at sensor locations
        if source_type == 'SAR':
            # SAR source profile
            src = amplitude *a3* torch.exp(-a4*x)   #shape ()
    ...
    def generate_internal_measurements(self, Tx, subdomain=(0, 1), t=None):
        """
        ...
        """
        if t is None:
            t = torch.linspace(subdomain[0], subdomain[1], self.n_sensors_thermal)
        # Analytical solution at x_meas over time t
        u = 1 - torch.exp(-t / Tx)
        
        return u
    
    def train_pinn(self, n_epochs=5000, n_colloc=200, n_bc0 = 40, n_interface = 50, n_therm = 50,lr=1e-3,source_type='gaussian', 
                   source_amplitude=0.0,Tx_range1=(0.3,0.7),Tx_range2=(1.0,1.5),x_meas=[0.2,0.8],x_interface = 0.5):
        """
        ...
        history = {
            'total': [], 'pde': [], 'bc0': [], 'meas': [], 'interface': [], 'flux': []
        }
        ...
        for epoch in range(n_epochs):
            ...
            for i in range(self.n_layers):
                DOMAIN = SUBDOMAINS[i]
                x_colloc = DOMAIN[0] + (DOMAIN[1] - DOMAIN[0]) * torch.rand(n_colloc)
                t_colloc =  DOMAIN[0] + (DOMAIN[1] - DOMAIN[0]) * torch.rand(n_colloc)
                xt_colloc = torch.stack([x_colloc, t_colloc], dim=1)

                # Generate thermal sensors and source sensors
                thermal_sensors = self.generate_internal_measurements(Tx[i], subdomain = DOMAIN)
                src_sensors = self.generate_source(source_type, source_amplitude, subdomain = DOMAIN)
                
                # Source values at collocation points
                src_colloc = self.generate_source(source_type, source_amplitude, a3[i], a4[i],subdomain = DOMAIN, x = x_colloc)
                
                # === PDE Loss ===
                residual = self.compute_pde_residual(xt_colloc, src_colloc, thermal_sensors,src_sensors, a1[i], a2[i])
                loss_pde += torch.mean(residual ** 2)
            
            # Compute thermal sensors and source sensors for BC0, measurement, interface and flux losses
            thermal_sensors = self.generate_internal_measurements(Tx[0])
            src_sensors = self.generate_source(source_type, source_amplitude,a3[0],a4[0])

            # === BC0 Loss ===
            t_bc0 = torch.linspace(self.domain[0], self.domain[1], n_bc0)
            x_bc0 = torch.zeros(n_bc0)
            xt_bc0 = torch.stack([x_bc0, t_bc0], dim=1)
            residual_bc0 = self.compute_bc0_residual(src_sensors, thermal_sensors, xt_bc0)
            loss_bc0 = torch.mean(residual_bc0 ** 2)
            
            # === Thermal Measurement Loss ===
            t_meas = torch.linspace(self.domain[0], self.domain[1], n_therm)
            xt_therm = torch.stack([x_meas[0]* torch.ones_like(t_meas), t_meas], dim=1)
            residual_meas = self.compute_thermal_meas_residual(src_sensors, thermal_sensors,x_meas[0],xt_therm,Tx[0])
            loss_meas = torch.mean(residual_meas ** 2)

            # === Interface Continuity Loss ===
            t_interface = torch.linspace(self.domain[0], self.domain[1], n_interface)
            xit = torch.stack([x_interface*torch.ones_like(t_interface), t_interface], dim=1)      # (n_interface_points, 2)
            residual_interface = self.compute_interface_continuity_loss(source_amplitude, source_type, a3, a4, Tx, xit)
            loss_interface = torch.mean(residual_interface ** 2)

            # === Flux Loss ===
            residual_flux = self.compute_flux_loss(source_amplitude, source_type, a3, a4, Tx,xit)
            loss_flux = torch.mean(residual_flux ** 2)

                    
            # Loss weights (no BC and IC weight needed - hard constraints!)

            if epoch == 0:
                initial_losses = torch.tensor([loss_pde.item(), loss_bc0.item(), loss_meas.item(), loss_interface.item(), loss_flux.item()])
            w_pde = 5.0 / (initial_losses[0])
            w_bc0 = 5.0 / (initial_losses[1])
            w_meas = 5.0 / (initial_losses[2])
            w_interface = 5.0 / (initial_losses[3])
            w_flux = 5.0 / (initial_losses[4])

            # === Total Loss (no BC1 loss - hard constraint!) ===
            loss_total = w_pde * loss_pde + w_bc0 * loss_bc0 + w_meas * loss_meas + w_interface * loss_interface + w_flux * loss_flux
            ...
            if epoch % 500 == 0 or epoch == n_epochs - 1:
                ...
                print(f"Epoch {epoch:5d} | Loss: {loss_total.item():.6f} | "
                      ...
                      f"Meas: {loss_meas.item():.6f} | Interface: {loss_interface.item():.6f} | "
                      f"Flux: {loss_flux.item():.6f} | LR: {current_lr:.2e}")
...
def plot_solution(model, Tx_test, x_meas_test, source_type='gaussian', source_amplitude=0.5, x_interface = 0.5, gt=None):
    ...
    for i in range(model.n_layers):
        src_i = model.generate_source(source_type, source_amplitude, a3[i], a4[i])
        if src_sensors is None:
            src_sensors = src_i
        else:
            src_sensors = torch.cat((src_sensors, src_i), dim=0)
    src_sensors = src_sensors[::2]
    ...
    # Predict solution
    with torch.no_grad():
        u_pred = model.forward(xt_grid, src_sensors, thermal_sensors)
    ...
    src_plot = None
    for i in range(model.n_layers):
        src_i = model.generate_source(source_type, source_amplitude, a3[i], a4[i], [0,1], x_plot).numpy()
        if src_plot is None:
            src_plot = src_i
        else:
            src_plot = np.concatenate((src_plot, src_i), axis=0)
    src_plot = src_plot[::2]
    ...
    ax1.axvline(x_interface, color='gray', linestyle='--', linewidth=2)  # Interface point
    ...
    ax3.plot(x_plot.numpy(), np.zeros_like(x_plot.numpy()), 'b--', linewidth=2, label='IC: u(x,0)')
    ax3.set_xlabel('x', fontsize=12)
    ax3.set_ylabel('Value', fontsize=12)
    ax3.set_title('Initial Condition and Source function', fontsize=12)
    ax3.legend()
    ...
    # Plot 5: u(x_meas,t) predicted vs measured one
    fig_4 = plt.figure(figsize=(6, 5))
    ax4 = fig_4.add_subplot(1, 1, 1)
    ax4.plot(t_plot.numpy(), u_pred[int(round(x_meas_test* 100)), :].squeeze(), label=f'Predicted u(x_meas,t)')
    u_meas_true = 1 - np.exp(-t_plot.numpy() / Tx_test)
    ax4.plot(t_plot.numpy(), u_meas_true, 'r--', label='Real u(x_meas,t)')
    ax4.set_xlabel('t', fontsize=12)
    ax4.set_ylabel(f'u({x_meas_test},t)', fontsize=12)
    ax4.set_title(f'Internal Measurement at x={x_meas_test}', fontsize=12)
    ax4.legend()
...
def main(cfg: DictConfig):
    ...
    history = model.train_pinn(
        ...
        n_interface=cfg.training.n_interface,
        ...
        Tx_range2=cfg.training.Tx_range3_multilayer
        # x_meas=cfg.training.x_meas,
        # x_interface = cfg.training.x_interface
    )
    ...
    model_path = os.path.join(cur_dir, "pinn_deeponet_heat_with_src_3branch_multilayer.pth")
    torch.save(model.state_dict(), model_path)
    print("✔ Model saved:", "pinn_deeponet_heat_with_src_multilayer.pth")

    # Plot results
    print("Generating plots...")
    
    fig1 = plot_training_history(history)
    fig1_path = os.path.join(cur_dir, "pinn_deeponet_training_with_src_3branch_multilayer.png")
    ...
    print("✔ Solution plot saved:", "pinn_deeponet_solution_with_src_multilayer.png")
    print("✔ Comparison plot saved:", "comparison_src_multilayer.png")
    print("✔ Internal measurement plot saved:", "internal_measurement_multilayer.png")
