...
class PINNDeepONet(nn.Module):
    ...
    def forward(self, ic_sensors, xt, src_sensors, thermal_sensors):
        ...
        b_meas_summed = b_meas.sum(dim=0, keepdim=True) # Sum over thermal sensors (batch, p)
        
        # Combine branch outputs
        b_combined = b_ic + b_src + b_meas_summed              # (batch, p)
    ...
    def train_pinn(self, n_epochs=5000, n_colloc=200, n_bc0 = 40, n_therm = 50,lr=1e-3, 
                   source_type='gaussian', source_amplitude=0.0, Tx_range1=(0.0,0.5), Tx_range2 = (0.5,1.0),
                   Tx_range3 = (1.0,1.5), x_meas=[0.2, 0.5, 0.8]):
        """
        ...
        for epoch in range(n_epochs):
            # Extraction of random Tx values for each internal measurement sensor
            ...
            ic_sensors = self.generate_ic()                     #[n_sensors_ic,]
...
def plot_solution(model, Tx_test, x_meas_test, source_type='gaussian', source_amplitude=0.5, gt=None):
    ...
    ax1.axvline(x = 0.5, color='gray', linestyle='--', linewidth=2)  # Interface point
...
def main(cfg: DictConfig):
    ...
    fig3,fig_err,fig_4 = plot_solution(model, Tx_test = cfg.test.Tx_test_multilayer_multiple_meas, x_meas_test = cfg.test.x_multiple_meas_test,source_type=cfg.source.source_type,
