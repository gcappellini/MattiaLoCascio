...
class PINNDeepONet(nn.Module):
    ...
    def forward(self, ic_sensors, xt, coeff_sensors, src_sensors):
        """
        Args:
        ...
        single_sample_coeff = coeff_sensors.dim() == 1
        if single_sample_coeff:
            coeff_sensors = coeff_sensors.unsqueeze(0)
        
        # Encode IC and source
        b_ic = self.branch_ic(ic_sensors)       # (batch, p)
        b_src = self.branch_source(src_sensors) # (batch, p)
        b_coeff = self.branch_coeff(coeff_sensors) # (batch, p)
        b_coeff_sum = b_coeff.sum(dim = 0, keepdim = True) # (1, p), if keepdim = False -> (p,)
        
        # Combine branch outputs
        b_combined = b_ic + b_coeff_sum + b_src               # (batch, p)
        ...
        if single_sample_ic and single_sample_src:
            u = bc_factor * u_net  
        else:
            u = bc_factor.unsqueeze(0) * u_net # (batch, n_points)
        
        return u.squeeze(0) if single_sample_ic and single_sample_src else u
    
    def compute_pde_residual(self, ic_sensors, xt, src_values, src_sensors, coeff_sensors, a1 = 18.992, a2=34185.667, wb=0.0005):
        """
        ...
        # Forward pass
        u = self.forward(ic_sensors, xt_requires_grad, coeff_sensors , src_sensors)  # (n_points,)
    ...
    def compute_bc0_residual(self, ic_sensors, src_sensors, xt, coeff_sensors, a5=175):
        """
        ...
        # Forward pass
        u = self.forward(ic_sensors, xt_requires_grad, coeff_sensors, src_sensors)
    ...
    def compute_coefficients(self, x = None, L0 = 0.07, t_span = 1800.035, c = [2348,3421], ro = [911,1090], k = [0.21,0.49],
        ...
        i = torch.clamp((x * self.n_layers).long(), 0, self.n_layers - 1)
    ...
    def train_pinn(self, n_epochs=5000, n_colloc=200, n_bc0 = 40, n_ic = 50, lr=1e-3, source_type='gaussian', source_amplitude=0.0):
        ...
        for epoch in range(n_epochs):
            ...
            a1_sensors, a2_sensors, a3_sensors, a4 = self.compute_coefficients() 
            coeff_sensors = torch.stack([a1_sensors, a2_sensors, a3_sensors], dim = 0) # (3, n_sensors_coeff)
            ...
            # === PDE Loss ===
            residual = self.compute_pde_residual(ic_sensors,  xt_colloc, src_colloc, src_sensors, coeff_sensors)
            ...
            residual_bc0 = self.compute_bc0_residual(ic_sensors, src_sensors, xt_bc0, coeff_sensors)
...
def plot_solution(model, source_type='gaussian', source_amplitude=0.5, gt=None):
    ...
    a1_sensors, a2_sensors, a3_sensors, a4 = model.compute_coefficients()
    coeff_sensors = torch.stack([a1_sensors, a2_sensors, a3_sensors], dim = 0) # (3, n_sensors_coeff)
    ...
    # Predict solution
    with torch.no_grad():
        u_pred = model.forward(ic_sensors, xt_grid, coeff_sensors, src_sensors)
    ...
    ax4.set_title(f'Solution Evolution in time', fontsize=12)
    ax4.legend()
...
def main(cfg: DictConfig):
    ...
    model_path = os.path.join(cur_dir, "prove_multilayer.pth")
    torch.save(model.state_dict(), model_path)
    print("✔ Model saved:", "prove_multilayer.pth")

    # Plot results
    print("Generating plots...")
    
    fig1 = plot_training_history(history)
    fig1_path = os.path.join(cur_dir, "prove_multilayer_training.png")
    fig1.savefig(fig1_path, dpi=150, bbox_inches='tight')
    print("✔ Training plot saved:", "prove_multilayer_training.png")
    ...
    fig2_path = os.path.join(cur_dir, "prove_multilayer_solution.png")
    fig_err_path = os.path.join(cur_dir, "prove_multilayer_comparison.png")
    fig2.savefig(fig2_path, dpi=150, bbox_inches='tight')
    fig_err.savefig(fig_err_path, dpi=150, bbox_inches='tight')
    print("✔ Solution plot saved:", "prove_multilayer_solution.png")
    print("✔ Comparison plot saved:", "prove_multilayer_comparison.png")
