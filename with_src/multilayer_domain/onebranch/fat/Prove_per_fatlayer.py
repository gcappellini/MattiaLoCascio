...
class PINNDeepONet(nn.Module):
    ...
    def forward(self, xt):
        ...
        first_layer_sensors = 0 * torch.ones_like(self.sensor_x_ic)  # Dummy sensors for first layer
        if first_layer_sensors.ndim == 1:
            single_sample_first = True
            first_layer_sensors = first_layer_sensors.unsqueeze(0)  # (1, n_sensors_ic)
        b_first_layer = self.branch_ic(first_layer_sensors)  # (1, p) 
        
        # Encode spatiotemporal locations
        tau = self.trunk(xt)                    # (n_points, p)
        
        # DeepONet operation: (batch, p) @ (p, n_points) = (batch, n_points)
        u_net = torch.matmul(b_first_layer, tau.T) + self.bias  # (batch, n_points)
...
def main(cfg: DictConfig):
    ...
    gt_data = np.loadtxt(to_absolute_path('gt_bioheat1D_src_fat_layer.csv'))

    #Save model
    cur_dir = os.getcwd()
    print("Current working directory:", cur_dir)

    model_path = os.path.join(cur_dir, "prove_fat_layer.pth")
    torch.save(model.state_dict(), model_path)
    print("✔ Model saved:", "prove_fat_layer.pth")

    # Plot results
    print("Generating plots...")
    
    fig1 = plot_training_history(history)
    fig1_path = os.path.join(cur_dir, "prove_fat_layer_training.png")
    fig1.savefig(fig1_path, dpi=150, bbox_inches='tight')
    print("✔ Training plot saved:", "prove_fat_layer_training.png")
    ...
    fig2_path = os.path.join(cur_dir, "prove_fat_layer_solution.png")
    fig_err_path = os.path.join(cur_dir, "prove_fat_layer_comparison.png")
    fig2.savefig(fig2_path, dpi=150, bbox_inches='tight')
    fig_err.savefig(fig_err_path, dpi=150, bbox_inches='tight')
    print("✔ Solution plot saved:", "prove_fat_layer_solution.png")
    print("✔ Comparison plot saved:", "prove_fat_layer_comparison.png")
