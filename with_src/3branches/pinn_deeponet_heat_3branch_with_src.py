...
class PINNDeepONet(nn.Module):
    ...
    def train_pinn(self, n_epochs=5000, n_colloc=200, n_bc0 = 40, n_ic = 50, n_therm = 50,lr=1e-3, theta_gt_range=(0.7, 1.0), 
        ...
        for epoch in range(n_epochs):
            ...
            u_therm_pred = self.forward(ic_sensors, xt_therm, src_sensors, thermal_sensors)
            u_therm_true = self.generate_internal_measurements(Tx, t_meas)
            loss_meas = torch.mean((u_therm_pred - u_therm_true) ** 2)
            ...
            if epoch % 500 == 0 or epoch == n_epochs - 1:
                ...
                print(f"Epoch {epoch:5d} | Loss: {loss_total.item():.6f} | "
                      ...
                      f"LR: {current_lr:.2e} | Meas: {loss_meas.item():.6f}")
