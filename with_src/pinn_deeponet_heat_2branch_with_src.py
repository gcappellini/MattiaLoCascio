...
class PINNDeepONet(nn.Module):
    ...
    def forward(self, ic_sensors, xt, src_sensors):
        """
        Args:
    ...
    def compute_pde_residual(self, ic_sensors, xt, src_values, src_sensors, a1=18.992, a2=34185.667, wb=0.0005):
        """
    ...
    # def compute_bc0_residual(self, ic_sensors, src_sensors, xt, a5=175, y3_0=0.0):
    #     """
    ...
    def compute_bc0_residual(self, ic_sensors, src_sensors, xt, a5=175):
        """
        Compute bc0 residual: R = u_t - u_xx - f(x)
    ...
    def train_pinn(self, n_epochs=5000, n_colloc=200, n_bc0 = 40, n_ic = 50, lr=1e-3, 
        ...
        for epoch in range(n_epochs):
            ...
            # === PDE Loss ===
            residual = self.compute_pde_residual(ic_sensors,  xt_colloc, src_colloc, src_sensors)
            ...
            u_ic_pred = self.forward(ic_sensors, xt_ic, src_sensors)
...
# ------- Plotting ------- #
def plot_solution(model, theta_gt_test, theta0_0_test, source_type='gaussian', source_amplitude=0.5, gt=None):
    ...
    # Predict solution
    with torch.no_grad():
        u_pred = model.forward(ic_sensors, xt_grid, src_sensors)
    ...
    # Plot 5: u(x,t) vs time
    fig_4 = plt.figure(figsize=(6, 5))
    ax4 = fig_4.add_subplot(1, 1, 1)
    ax4.plot(t_plot.numpy(), u_pred[0, :], label=f'x={0.0}')
    ax4.set_xlabel('t', fontsize=12)
    ax4.set_ylabel('u(x,t)', fontsize=12)
    ax4.set_title(f'Solution Evolution in time (theta_gt,0_0={theta_gt_test,theta0_0_test})', fontsize=12)
    ax4.legend()
...
def plot_u_net_ic(model, theta_gt_test, theta0_0_test,
                  source_type='gaussian', source_amplitude=0.5):
    """Plot u_net(x,0) BEFORE multiplying by BC factor"""
    ...
    nx = 201
    x_plot = torch.linspace(model.domain[0], model.domain[1], nx)
    t_zero = torch.zeros(nx)
    xt_ic = torch.stack([x_plot, t_zero], dim=1)   # (nx, 2)
    
    with torch.no_grad():
        # Branch (aggiungo batch dim)
        b_ic  = model.branch_ic(ic_sensors.unsqueeze(0))        # (1, p)
        b_src = model.branch_source(src_sensors.unsqueeze(0))   # (1, p)
        b_combined = b_ic + b_src                               # (1, p)

        # Trunk
        tau = model.trunk(xt_ic)                                # (nx, p)

        # u_net(x,0) = b_combined @ tau^T + bias
        u_net_ic = torch.matmul(b_combined, tau.T) + model.bias # (1, nx)
        u_net_ic = u_net_ic.squeeze(0).cpu().numpy()            # (nx,)
    
    # Plot
    fig, ax = plt.subplots(figsize=(10, 6))
    ax.plot(x_plot.cpu().numpy(), u_net_ic, 'b-', linewidth=2, label='u_net(x,0) (raw)')
    ax.set_xlabel('x')
    ax.set_ylabel('u_net(x,0)')
    ax.set_title(f'u_net at t=0 (theta_gt={theta_gt_test}, theta0_0={theta0_0_test})')
...
def main(cfg: DictConfig):
    ...
    history = model.train_pinn(
        ...
        source_amplitude=cfg.source.source_amplitude
    )
    # Load ground truth data for comparison
    gt_data = np.loadtxt(to_absolute_path('gt_bioheat1D_src.csv'))

    #Save model
    cur_dir = os.getcwd()
    print("Current working directory:", cur_dir)

    model_path = os.path.join(cur_dir, "pinn_deeponet_heat_with_src.pth")
    torch.save(model.state_dict(), model_path)
    print("✔ Model saved:", "pinn_deeponet_heat_with_src.pth")

    # Plot results
    print("Generating plots...")
    
    fig1 = plot_training_history(history)
    fig1_path = os.path.join(cur_dir, "pinn_deeponet_training_with_src.png")
    fig1.savefig(fig1_path, dpi=150, bbox_inches='tight')
    print("✔ Training plot saved:", "pinn_deeponet_training_with_src.png")
    
    
    fig2,fig_err = plot_solution(model, theta_gt_test = 0.95238, theta0_0_test = 0.829, source_type=cfg.source.source_type, source_amplitude = cfg.source.source_amplitude, gt=gt_data)
    fig2_path = os.path.join(cur_dir, "pinn_deeponet_solution_with_src.png")
    fig_err_path = os.path.join(cur_dir, "comparison_src.png")
    fig2.savefig(fig2_path, dpi=150, bbox_inches='tight')
    fig_err.savefig(fig_err_path, dpi=150, bbox_inches='tight')
    print("✔ Solution plot saved:", "pinn_deeponet_solution_with_src.png")
    print("✔ Comparison plot saved:", "comparison_src.png")

    fig_u_net = plot_u_net_ic(model, 0.95238, 0.829,
                          source_type=cfg.source.source_type,
                          source_amplitude=cfg.source.source_amplitude)
    fig_u_net.savefig("u_net_ic_debug.png", dpi=150, bbox_inches="tight")
