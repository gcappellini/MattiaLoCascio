...
class PINNDeepONet(nn.Module):
    ...
    def compute_pde_residual(self, ic_sensors, xt, src_values, thermal_sensors, src_sensors, a1=18.992, a2=34185.667, wb=0.0005):
        ...
        Args:
            ...
            thermal_sensors: (n_sensors_thermal,) internal measurements at x = x_meas
    ...
    def compute_thermal_meas_residual(self,ic_sensors, src_sensors, thermal_sensors,x_meas,xt,Tx):
        ...
        u_meas = self.generate_internal_measurements(Tx, t)
    ...
    def generate_ic(self, theta_gt, theta0_0, X_gt=0.14286, a5=175, y3_0=0.0, x = None):
        """
        Generate null initial condition sensors.
        Returns:
    ...
    def train_pinn(self, n_epochs=5000, n_colloc=200, n_bc0 = 40, n_ic = 50, n_therm = 50,lr=1e-3, theta_gt_range=(0.7, 1.0), 
                   theta0_0_range=(0.6, 0.9), source_type='gaussian', source_amplitude=0.0,Tx_range=(0.3,1.5),x_meas=0.5):
        """
        ...
        for epoch in range(n_epochs):
            # Sample random values for theta_gt, theta0_0, Tx
            ...
            Tx = Tx_range[0] + (Tx_range[1] - Tx_range[0]) * torch.rand(1).item()

            # Generate IC, source and thermal sensors
            ic_sensors = self.generate_ic(theta_gt, theta0_0)
            src_sensors = self.generate_source(source_type, source_amplitude)
            thermal_sensors = self.generate_internal_measurements(Tx)
            ...
            x_meas = x_meas * torch.ones(n_therm)
            xt_therm = torch.stack([x_meas, t_meas], dim=1)
            residual_meas = self.compute_thermal_meas_residual(ic_sensors, src_sensors, thermal_sensors,x_meas,xt_therm,Tx)
            ...
            # === Total Loss (no BC1 and IC losses - hard constraints!) ===
...
def plot_solution(model, theta_gt_test, theta0_0_test, Tx_test, x_meas_test, source_type='gaussian', source_amplitude=0.5, gt=None):
    ...
    ax4.plot(t_plot.numpy(), u_pred[x_plot.numpy() == x_meas_test, :].squeeze(), label=f'Predicted u(x_meas,t)')
...
def main(cfg: DictConfig):
    ...
    history = model.train_pinn(
        ...
        Tx_range=cfg.training.Tx_range,
    ...
    model_path = os.path.join(cur_dir, "pinn_deeponet_heat_with_src_3branch.pth")
    ...
    fig1_path = os.path.join(cur_dir, "pinn_deeponet_training_with_src_3branch.png")
    ...
    fig3,fig_err,fig_4 = plot_solution(model, theta_gt_test = cfg.test.theta_gt_test, theta0_0_test = cfg.test.theta0_0_test, 
                                 Tx_test = cfg.test.Tx_test, x_meas_test = cfg.test.x_meas_test,source_type=cfg.source.source_type,
                                   source_amplitude = cfg.source.source_amplitude, gt=gt_data)
    fig3_path = os.path.join(cur_dir, "pinn_deeponet_solution_with_src_3branch.png")
    fig_err_path = os.path.join(cur_dir, "comparison_src_3branch.png")
    fig_4_path = os.path.join(cur_dir, "internal_measurement.png")
    ...
    print("✔ Internal measurement plot saved:", "internal_measurement.png")
