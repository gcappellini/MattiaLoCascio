...
class PINNDeepONet(nn.Module):
    ...
    def generate_ic(self, theta_gt, theta0_0, X_gt=0.14286, a5=1.1666667, y3_0=0.0, x = None):
        """
        Generate initial condition sensors 
        Returns:
...
def main(cfg: DictConfig):
    ...
    gt_data = np.loadtxt(to_absolute_path('gt_bioheat1D_without_src_sin.csv'))

    #Save model
    cur_dir = os.getcwd()
    print("Current working directory:", cur_dir)

    model_path = os.path.join(cur_dir, "pinn_deeponet_heat_without_src_sin.pth")
    torch.save(model.state_dict(), model_path)
    print("✔ Model saved:", "pinn_deeponet_heat_without_src_sin.pth")

    # Plot results
    print("Generating plots...")
    
    fig1 = plot_training_history(history)
    fig1_path = os.path.join(cur_dir, "pinn_deeponet_training_without_src_sin.png")
    fig1.savefig(fig1_path, dpi=150, bbox_inches='tight')
    print("✔ Training plot saved:", "pinn_deeponet_training_without_src_sin.png")
    
    fig2,fig_err = plot_solution(model, theta_gt_test = 0.95238, theta0_0_test = 0.829, gt=gt_data)
    fig2_path = os.path.join(cur_dir, "pinn_deeponet_solution_without_src_sin.png")
    fig_err_path = os.path.join(cur_dir, "comparison_sin.png")
    fig2.savefig(fig2_path, dpi=150, bbox_inches='tight')
    fig_err.savefig(fig_err_path, dpi=150, bbox_inches='tight')
    print("✔ Solution plot saved:", "pinn_deeponet_solution_without_src_sin.png")
    print("✔ Comparison plot saved:", "comparison_sin.png")
