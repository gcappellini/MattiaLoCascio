function [sol] = OneDimBH_no_src_sin()
  ...
  fileID = fopen('\Users\Mattia\Dropbox\Mattia\Tesi Magistrale/gt_bioheat1D_without_src_sin.csv', 'w');
  for i = 1:101
      for j = 1:101
...
colorbar;
saveas(fig, 'bioheat_1D_without_src_sin.png');
%view(2); % Top view for better visualization
  ...
  % --------------------------------------------------------------------------
  function theta0 = OneDimBHic(x)
      [theta0, ~, ~, ~] = ic_bc_sin(x, 0);
