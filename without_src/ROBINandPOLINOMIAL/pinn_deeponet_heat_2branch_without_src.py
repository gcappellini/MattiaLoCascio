...
class PINNDeepONet(nn.Module):
    ...
    def __init__(self, n_sensors_ic=20, n_sensors_src=20, 
        ...
        self.p = p
        self.domain = [0.0, 1.0]  # Spatial domain and temporal domain
    ...
    def forward(self, ic_sensors, xt, src_sensors = None):
        ...
        single_sample = ic_sensors.dim() == 1 #True if ic_sensors has only one dimension (n_sensors_ic,)
        if single_sample:
            ic_sensors = ic_sensors.unsqueeze(0) # (1, n_sensors_ic)
        
        # Encode IC and source
        b_ic = self.branch_ic(ic_sensors)       # (batch, p)
        
        # Combine branch outputs
        b_combined = b_ic                      # (batch, p)
    ...
    def train_pinn(self, n_epochs=5000, n_colloc=200, n_bc0 = 40, n_ic = 50, lr=1e-3, 
        ...
        Loss = w_pde * L_pde + w_ic * L_ic + w_bc0 * L_bc0
        Note: BC1 is enforced as hard constraint, no soft BC1 loss needed
        """
        print("="*70)
...
def main(cfg: DictConfig):
    ...
    gt_data = np.loadtxt(to_absolute_path('gt_bioheat1D_without_src_poly.csv'))

    #Save model
    cur_dir = os.getcwd()
    print("Current working directory:", cur_dir)

    model_path = os.path.join(cur_dir, "pinn_deeponet_heat_without_src_poly.pth")
    torch.save(model.state_dict(), model_path)
    print("✔ Model saved:", "pinn_deeponet_heat_without_src_poly.pth")

    # Plot results
    print("Generating plots...")
    
    fig1 = plot_training_history(history)
    fig1_path = os.path.join(cur_dir, "pinn_deeponet_training_without_src_poly.png")
    fig1.savefig(fig1_path, dpi=150, bbox_inches='tight')
    print("✔ Training plot saved:", "pinn_deeponet_training_without_src_poly.png")
    
    fig2,fig_err = plot_solution(model, theta_gt_test = 0.95238, theta0_0_test = 0.829, gt=gt_data)
    fig2_path = os.path.join(cur_dir, "pinn_deeponet_solution_without_src_poly.png")
    fig_err_path = os.path.join(cur_dir, "comparison_poly.png")
    fig2.savefig(fig2_path, dpi=150, bbox_inches='tight')
    fig_err.savefig(fig_err_path, dpi=150, bbox_inches='tight')
    print("✔ Solution plot saved:", "pinn_deeponet_solution_without_src_poly.png")
    print("✔ Comparison plot saved:", "comparison_poly.png")
